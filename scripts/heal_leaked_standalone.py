...
def heal_leaked_call_text(text):
    ...
    stats = {"anchors": 0, "invokes": 0, "fields": 0, "suspicious": 0}
...
def main():
    ...
    totals = {"anchors": 0, "invokes": 0, "fields": 0, "suspicious": 0}
    touched = 0
    for proj_dir in dirs:
        for md in sorted(proj_dir.glob("*.md")):   # нерекурсивно: daily/archive вне обхода
            ...
            print("{}/{}: якорей {}, полей {}, invoke {}".format(
                proj_dir.name, md.name, stats["anchors"], stats["fields"], stats["invokes"]))
            if apply:
                md.write_text(fixed, encoding="utf-8")
    print("\nИтого: статей {}; якорей {}, полей {}, invoke {}, подозрительных {}{}".format(
        touched, totals["anchors"], totals["fields"], totals["invokes"],
        totals["suspicious"], "" if apply else " [dry-run, ничего не записано]"))
    return 0
