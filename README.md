...
**Тюнинг ранжирования** (`RRF_K`, `DECAY_WEIGHT`, `BM25_TITLE_B`/`BM25_TAGS_B`/`BM25_BODY_B`) — всё применяется на этапе запроса, поэтому смена не требует ни пересборки индекса, ни ре-эмбеддинга: один эксперимент ≈ минута. Сетка: `scripts/eval_ranking.py`.

Прогон сетки на 137 реальных запросах (2026-07-19) улучшений не нашёл — **дефолты оставлены**: `RRF_K` 10/20/60 дают одинаковый результат (плоская зона), на 120 recall@10 падает 0.65 → 0.49; `b=1.0` выигрывает один запрос из 137, то есть шум.
