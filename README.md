...
Персональная база знаний для AI-ассистентов. MCP-сервер на Synology NAS с гибридным поиском, автокомпиляцией дневных логов в wiki-статьи и веб-интерфейсом.
...
└──────────────────┘
         ...
         ▼
┌─────────────────────────────────────────────┐
│  knowledge/  (Docker volume на NAS)         │
│  ├── daily/          дневные логи           │
│  ├── project-a/      проект A               │
│  ├── project-b/      проект B               │
│  ├── ...             (настраиваемые)        │
...
| Контейнеризация | Docker на Synology NAS (DS220+) |
...
```
memory-compiler/
├── server.py              # весь серверный код (2235 строк)
├── requirements.txt       # 6 зависимостей
├── Dockerfile             # Python 3.12-slim + git
├── docker-compose.yml     # volume для knowledge + кэш HuggingFace
├── deploy.py              # деплой на NAS через SSH/paramiko
├── deploy_image.py        # сборка Docker-образа для NAS
├── deploy.bat             # Windows batch wrapper
├── test_all.py            # интеграционные тесты (73 проверки)
├── README.md              # этот файл
└── knowledge/             # база знаний (Docker volume)
    ├── index.md
    ├── daily/
    ├── {project}/
    │   ├── article.md
    │   ├── _session.md        # контекст последней сессии
    │   └── _active_context.md # лента последних 10 действий
    ├── .whoosh_index/
    ├── .embeddings.pkl
    └── .article_meta.json
```
...
```bash
# Автоматический деплой из Windows (SSH + paramiko):
cd /path/to/memory-compiler
python deploy.py

# Принудительный rebuild (если обновился server.py):
# deploy.py НЕ делает --rmi, поэтому для обновления кода:
ssh <user>@<your-host>
cd /path/to/memory-compiler
...
Время полного rebuild: ~2-3 минуты (скачивание pip-зависимостей + модель sentence-transformers при первом запуске).
