...
- **Cross-encoder reranking** — `BAAI/bge-reranker-v2-m3` (multilingual, built on BGE-M3, сильный RU+EN) пересортировывает топ-20 кандидатов для финального выбора (precision@3 +15-20%); опционально SPLADE 3-way через `SPLADE_ENABLED=true`

```
hybrid_score = (0.4 × BM25 + 0.6 × semantic) × decay_factor
final = cross_encoder.rerank(hybrid_top_20) → top_3
```
...
16 REST endpoints (`/api/*`): health, версия, логин/auth, поиск, сохранение, CRUD статей, проекты, граф знаний, аналитика, теги, компиляция (превью/запуск), экспорт, аудит. Плюс `/` (Web UI), `/login`, `/sse` (MCP-транспорт).
