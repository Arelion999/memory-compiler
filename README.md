...
- **Ответы** — вопрос по базе: тот же конвейер, что у tool `ask` (широкий пул → cross-encoder реранк → фолбэк на все проекты), выдача — релевантные фрагменты с указанием статьи-источника. Генерации нет: LLM на сервере отсутствует, это retrieval с источниками, а не сочинённый ответ
