...
14 endpoints: поиск, сохранение, CRUD, экспорт проекта, граф знаний, аналитика, теги, компиляция.
...
```
1. Начало задачи    → search("тема") + load_session("project")
2. В процессе       → ask("как настроить X?")
3. Решение найдено  → save_lesson(...)     ← автоматически
4. Конец сессии     → save_session(...)
```
