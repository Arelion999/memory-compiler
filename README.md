...
19 REST endpoints (`/api/*`): health, версия, логин/auth, поиск, семантически похожие статьи, таймлайн версий факта, сохранение, CRUD статей, проекты, граф знаний, аналитика, теги, компиляция (превью/запуск), экспорт, аудит, логи. Плюс `/` (Web UI), `/login`, `/sse` (MCP-транспорт).
