...
Прироста нет (сдвиг — по одному запросу на уровень, то есть шум) при 32-кратной цене. `recall@10` совпал структурно: пул кандидатов равен 10, поэтому reranker лишь переставляет те же 10 документов. Польза cross-encoder берётся из переранжирования большого пула (50–100) в короткий топ; на этом CPU такой пул недостижим (14.5 с уже за 10 кандидатов). Включать имеет смысл только вместе с бо́льшим `SEARCH_CANDIDATE_POOL` и/или лёгкой моделью — и обязательно с повторным замером тем же харнессом.
