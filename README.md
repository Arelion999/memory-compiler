...
7. An update to matching tracking articles (version, IP, port, URL) — the current value is owned by tracking, with a guard against version regression; a version the note itself calls a plan, a requirement, not yet done or a condition (`< 0.4.0`) is not taken, nor is a bare version that follows only the name of another product (`Electron 44.2.0`, `mcp[cli]==1.28.1`); an IP field changes only when the relevant text names exactly one address of the same role
8. A git commit
...
**The model writes the quote.** Only whoever saw the output can propose a quote, so the model adds it: after a note about a node the agent worked with in this session, the client hook asks for a `verify` line. Picking the quote from command history was measured and rejected — a whole read-only command to the node right before writing about it appeared in 3.3% of cases. The `POST /api/probe` target is accepted as a list (up to three): a node is called both by connection name and by address. A note carries `targets` — those of the requested targets the article was actually found by; without them a quote from an article about one node would apply to another and produce a false `stale`. The verdict requires full equality of the command: `docker ps -a` is no longer `docker ps`, and on the quote `docker ps` it would report "verified" precisely on a stale fact.
...
- **Secrets are listed, marked, and openable.** A search that matches an encrypted article shows it with a lock. Nothing is decrypted until you click — so the click itself is the deliberate reveal, the same bargain the Web UI's "Show" button makes. (Since v1.87.0 no result carries a `memory://` resource link at all, secret or not — the panel opens any of them the same way, via `read_article(project, file)`.)
...
- **Theme and palette come from the host, not the OS.** `prefers-color-scheme` inside the sandboxed iframe reflects the Windows theme, not Claude Desktop's, so the panel could render light inside a dark chat. The view applies `theme` and the standard `--color-*` variables from `hostContext.styles.variables` (on `ui/initialize` and in `host-context-changed`), keeping its own colours only as fallbacks.
...
The server itself warns a session about other sessions' writes to the project and reminds it about `session_note` after long work without a write. It tracks these snapshots per MCP session — and Claude Desktop, when memory-compiler is also listed in `claude_desktop_config.json`, may hand its own server instance to Code-tab chats, i.e. one MCP session for all of them. To let the server tell chats apart, the client passes the chat id in any call as the service argument `_client_session`: the server strips it before the tool runs and keeps it out of the audit log, and without it behaves as before. In Claude Code a PreToolUse hook on `mcp__memory-compiler__.*` does this by returning `updatedInput` — the original arguments plus `_client_session` = `session_id`; no `permissionDecision` is needed. The parameter is declared in every tool's schema as an optional service field: an argument missing from the schema never gets through the Claude Desktop bridge to the server (verified on Desktop 1.52386 with Claude Code 2.1.266). ⚠️ Desktop caches the tool list for the lifetime of the app — restart it fully after updating the server.
