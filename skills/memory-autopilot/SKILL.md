...
### Размер payload — ~1500 символов по умолчанию, до ~4000 когда оправдано

**По умолчанию держи `content` около 1500 символов ОБЫЧНОГО текста.** До ~4000
можно, когда текст действительно неделимый: один сюжет — механизм, доказательства
и решение по одной теме. Если внутри несколько тем, это несколько статей, и дробить их надо
независимо от размера — закопанный в чужой `content` разбор не всплывает в поиске,
потому что ранжируются заголовки.

**Что происходит при перерасходе.** Аргументы вызова обрываются НА ГЕНЕРАЦИИ:
JSON приходит незакрытым (нет закрывающей кавычки и `}`), клиент отбивает его с
`InputValidationError` ещё до отправки на сервер. Признаки: `raw` в тексте ошибки
кончается посреди слова, а `len` от попытки к попытке РАЗНАЯ — значит это не
фиксированный потолок на длину аргумента, а обрыв потока.

**Механизм.** Токены размышления делят бюджет `max_tokens` с текстом и с
...
Поэтому длинный разбор причины падения оставляет всё меньше места самому вызову —
повторять тот же payload бесполезно. Кириллица дорогая: 1500 символов это уже
порядка тысячи токенов.

**Счёт идёт по СГЕНЕРИРОВАННОМУ виду, а не по читаемому тексту.** JSON допускает
кириллицу и литералом, и escape-последовательностями `\uXXXX`. В escaped-форме
одна буква занимает ШЕСТЬ символов payload вместо одного — тот же текст съедает
бюджет вшестеро быстрее, и вызов рвётся уже на ~400 символах содержания. Форма
выбирается не глобально на вызов, а по-разному для разных полей: в коротких
строковых полях (`tags`) escape дёшев и вылезает часто, в `content` — фатален.
Наблюдалось вживую 2026-07-30: при поднятом до 64000 потолке вызов оборвался на
~2.4 КБ escaped-payload, а повтор тем же по смыслу текстом в обычной форме прошёл
сразу.

**Отличать на глаз, по `raw` в тексте ошибки.** Читаемая кириллица — обычный
случай, планка ~1500. Сплошная лента `\uXXXX` вместо букв — escaped-форма, и
реальный потолок СОДЕРЖАНИЯ примерно вшестеро ниже. Отсюда же правило повтора: если тот
же по смыслу текст со второго раза прошёл, изменилась ФОРМА, а не длина — не
считай это случайностью и не делай вывод, что лимита нет.

**Потолок конфигурируем.** `CLAUDE_CODE_MAX_OUTPUT_TOKENS` в блоке `env` файла
`~/.claude/settings.json`: дефолт 32000, документированный максимум 64000,
применяется при перезапуске приложения. На дефолтной машине верхняя планка ближе
к 1500, на поднятом потолке ~4000 проходит спокойно.

**Провал дешёвый — этим можно пользоваться.** Упавший вызов до сервера не доходит
и в аудит-лог не попадает вообще («записи нет» = «не дошло», а не «сервер не
справился»), ничего не записывается наполовину. Поэтому допустимо попробовать
больший payload и разделить его при отказе. Чего делать НЕЛЬЗЯ — повторять тот же
объём.
...
| «Уложу весь разбор в один finish_task» | ~1500 по умолчанию, до ~4000 если тема одна. Несколько тем — несколько статей |
| «Вызов упал, повторю тем же payload» | Упадёт снова. Дроби, а не повторяй |
