...
_HEADER_META_PREFIXES = (
    ...
    # «Проверено» — чем подтверждён факт (v1.71.0). Без записи ЗДЕСЬ метка утекла
    # бы в тело статьи: в превью поиска, в индекс и в ИИ-контексты.
    "**Проверено:**",
)
...
def detect_contradictions(new_content: str, project: str, exclude_path: Optional[str] = None) -> list[str]:
    ...
    ⚠️ ОТКЛЮЧЁН ОТ save_* с v1.54.1 — не подключать обратно, не разобравшись.
    Четыре причины, по которым он не окупался (замер на живой базе 2026-08-11):
      • сигнал никуда не вёл: только строка в тексте ответа. Ни пометки на статье,
        ни задачи, ни записи в журнал — прочитала модель или нет, следа не остаётся;
      • задача уже решена: auto_update_tracking/save_tracking_article не
        предупреждают, а ОБНОВЛЯЮТ значение, и у них есть авторитет по времени,
        которого здесь нет (детектор не знает, какая из двух статей свежее). Хендлер
        это и признавал, выбрасывая ip/порт/url/версию при сработавшем tracking;
      • точность: «одна сущность» = вхождение generic-слова из _ENTITY_KEYWORDS
        (vpn, ssh, prod…), а «факт» = совпадение регулярки. Поэтому боевой адрес
        сравнивался с ПРИМЕРАМИ из текста — ловились пары с 192.0.2.x (RFC 5737
        TEST-NET-1, документационная сеть). Выхлоп: 10% статей проекта на 225
        файлах, 28% на 274, все про IP, истинных среди размеченных не нашлось;
      • цена: читает ВСЕ *.md проекта на каждое сохранение (медиана 149/213 мс,
        max 352 мс, растёт линейно), причём синхронно внутри async-хендлера.
    Функция и её тесты оставлены: если ревизия базы понадобится, ей место в отчёте
    ПО ТРЕБОВАНИЮ (lint/stale_facts), где выдачу смотрят осознанно и разом.
...
_FACT_PATTERNS = {
    ...
    # соседями: version и ip обвешаны гардами ровно от таких коллизий — порт остался
    # наивным. Двоеточие считается портовым, только когда слева стоит ХОСТ (localhost,
    # IPv4 или доменное имя с точкой); плюс явная словесная форма 'port/порт N'.
    # Ровно ОДНА группа захвата: extract_facts_from_text на findall с несколькими
    # группами берёт m[0] и получил бы пустую строку от несработавшей альтернативы.
    "port": re.compile(
        r'(?:\b(?:port|порт)\s*[:=]?\s*'
        r'|(?:localhost|\d{1,3}(?:\.\d{1,3}){3}|[A-Za-z0-9-]+(?:\.[A-Za-z]{2,})+):)'
        r'(\d{2,5})\b',
        re.IGNORECASE,
    ),
...
# Пара к _HISTORICAL_MARKERS (v1.92.4): те отсекают прошлое, эти — ещё не наступившее.
...
#
# ⚠️ ПОМЕТКА ЖИВЁТ ТОЛЬКО В САМОЙ отменённой статье, а не в отдельном индексе:
# читающий видит предупреждение, даже открыв файл напрямую, и не нужен обход всех
# статей на каждое чтение (ровно та цена, из-за которой detect_contradictions
# отключили в v1.54.1). Обратная сторона не хранится, а вычисляется по пометке:
# attach_corrections подтягивает поправку к найденной отменённой статье,
# project_corrections собирает поправки проекта. Метки «Отменяет» в самой поправке
# нет сознательно (решение 13.09.2026): опасно читать отменённую статью, а не
# поправку, а вторая копия связи расходилась бы с первой. Константу под неё из
# v1.69.0 не писал и не читал никто — удалена.
...
# ─── Журнал сессий и открытые вопросы живут в journal (v1.81.0) ───────────
# Реэкспорт, а не переезд по вызывающим: handlers берёт эти имена из storage
# модульным импортом, maintenance — `from memory_compiler.storage import
# add_question, _session_path`, а тесты домена ходят через `storage.<имя>`, в том
# числе к приватным (_session_path, _split_session_blocks, _questions_path,
# parse_questions). Ломать этот адрес ради разреза файла незачем.
#
# ⚠️ ИМПОРТ В КОНЦЕ ФАЙЛА, И ЭТО НЕ УКРАШЕНИЕ. journal тянет хелперы storage
# (safe_project_dir, project_dir, normalize_project) ОТЛОЖЕННО, внутри функций —
# цикла storage ↔ journal нет, а тестам остаётся возможность патчить имя на
# модуле-владельце. Перенесёшь этот импорт в шапку — цикл не появится, но
# смысла тоже не будет: имена нужны только потребителям storage.
#
# ⚠️ КОНСТАНТЫ РЕЭКСПОРТИРУЮТСЯ ПО ЗНАЧЕНИЮ. `storage.MAX_SESSIONS` — копия
# ссылки, и патч `storage.MAX_SESSIONS` поведения journal НЕ изменит. Сейчас
# тесты их только читают; понадобится патчить — патчить journal.
from memory_compiler.journal import (  # noqa: E402,F401
    MAX_SESSIONS, SESSION_SEP, MAX_NOTES, RUNNING_MARK,
    _session_path, _split_session_blocks, _running_notes, running_notes_today,
    append_note, append_session, latest_session,
    _questions_path, _q_key, parse_questions, _write_questions,
    _PLAN_START, _PLAN_ENUM, is_plan_list,
    add_question, close_questions, open_questions_list,
    _REFLECTION_ACTION_VERBS, _NEGATION_RE, extract_reflections, append_reflections,
    _REFL_JUNK, _reflection_lines, relevant_reflections,
)
