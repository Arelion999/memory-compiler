...
(function () {
  ...
  function renderList() {
    ...
    // проекту, напоминание о заметке) приезжают полем notice ЕДИНСТВЕННОГО JSON:
    // с v1.87.0 у search нет отдельного текстового блока вовсе, есть только он.
    // Не показать их здесь значило бы отдать подсказку модели и спрятать от
    // человека, молча.
    ...
    // Диагностика — только когда хосту есть что сказать (см. hostSpoke ниже).
    if (hostSpoke(hostDiag)) root.appendChild(renderDiag());
  ...
  // ⚠️ ПОКАЗЫВАЕМ, ТОЛЬКО КОГДА ХОСТ ЗАГОВОРИЛ (v1.75.1). В v1.74.3 блок рисовался
  // всегда и на Claude Desktop 1.49585 сообщал «НЕТ / не прислан / нет» — место
  // занимала ровно та ситуация, где информации ноль. Удалить его совсем дороже:
  // клиент кэширует HTML вьюхи на всю свою жизнь, поэтому каждая проверка панели
  // стоит полного перезапуска приложения, и без зонда смена поведения хоста
  // заметится случайно. С перевёрнутым условием блок появится САМ, когда хост
  // начнёт слать контекст, — и это будет сигналом, что пора включать поддержку
  // темы, которая с v1.74.2 лежит и спит.
  //
  // ⚠️ Предикат ЧИСТЫЙ и берёт diag аргументом: так его прогоняет node в тестах,
  // а внутри песочного iframe консоли всё равно нет — проверить логику иначе
  // нечем.
  function hostSpoke(diag) {
    if (!diag) return false;
    if (diag.init && Object.keys(diag.init).length) return true;
    if (diag.changes) return true;
    return !!(diag.vars && Object.keys(diag.vars).length);
  }
