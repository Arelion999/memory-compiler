...
async def list_tools() -> list[Tool]:
    tools = [
        ...
        Tool(
            name="save_tracking",
            description="Создать или обновить tracking-статью (снимок текущего состояния). Переданные факты сливаются с текущим снимком, прежний снимок уходит в history[]. Используй для 'текущая версия', 'текущий деплой' и т.д.",
            inputSchema={
                "type": "object",
                "properties": {
                    "project": {"type": "string", "description": "Имя проекта"},
                    ...
                    # Слияние (v1.93.0): откат версии записью {version} стёр container,
                    # port и deploy у tracking/deployment (живой случай 25.09.2026).
                    # v1.96.0: поля релиза возвращаются вместе с бывшей версией — агент узнаёт
                    # об этом только из описания (канал до модели — description).
                    "facts": {"type": ["object", "string"],
                              "description": "Факты: {version: '1.3.50', url: ...}. "
                                             "Переданные поля заменяются, остальные остаются; "
                                             "null удаляет поле. При смене version поля релиза "
                                             "(commit, tag, tests, verified…) без новых значений "
                                             "уходят в историю, а при возврате к прежней version "
                                             "возвращаются из её снимка. Можно строкой — она "
                                             "ляжет полем note"},
                    "narrative": {"type": "string", "description": "Опциональное описание (иначе автогенерация)"},
                    # Без поля default (v1.90.0): оно заставляет клиента требовать параметр
                    # явно. Значение по умолчанию названо в описании.
                    "replace": {"type": "boolean",
                                "description": "true — заменить снимок целиком переданными "
                                               "фактами, прочие поля уйдут в историю "
                                               "(по умолчанию false — слияние)"}
                },
