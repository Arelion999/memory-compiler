...
# Инструменты, означающие СМЕНУ РАБОТЫ: после них открытие статьи относится уже к
# другому намерению, а не к последнему поисковому запросу. Закрывают ожидание так же,
# как это делает новый запрос.
#
# Почему признак СОБЫТИЙНЫЙ, а не временной. Разрыв «запрос → открытие» имеет длинный
# хвост (медиана 109 с, 75-й процентиль 2311 с, максимум 42 часа), и напрашивалось
# окно по времени. Но естественного разлома в распределении НЕТ, а подбирать порог по
# тому, как растёт метрика, — круговое рассуждение. Классификация же по типу события
# разделяет данные сама, и очень резко (замер 2026-07-19 на живом логе, 455 пар):
#     между запросом и открытием НЕ было смены работы — 276 пар, медиана разрыва 12 с,
#                                                       95-й 145 с, максимум 552 с;
#     смена работы БЫЛА                              — 179 пар, медиана разрыва 13475 с
#                                                       (3.7 часа), 78.8% дольше получаса.
# Двенадцать секунд против трёх с половиной часов, и НИ ОДНА пара без смены работы не
# выходит за 10 минут. Поэтому порог по времени не нужен: тип события отделяет чтение
# выдачи от возврата к базе по новому поводу.
#
# Устойчивость проверена: исключение самого частого и самого спорного разделителя
# (edit_article — статью могли поправить и вернуться к той же выдаче) меняет результат
# на ОДНУ пару из 276, медиана и максимум те же. Вывод не держится на спорном члене.
#
# Просмотр самой выдачи разделителем НЕ считается: 237 пар, где между запросом и
# открытием были только другие read_article, имеют медиану 11 с и максимум 527 с —
# это одно и то же намерение, листание результатов.
_BOUNDARY_TOOLS = frozenset({
    "start_task", "finish_task",
    "save_lesson", "save_decision", "save_runbook", "save_secret", "save_tracking",
    "save_from_template", "save_session", "save_compact",
    "edit_article", "delete_article", "compile", "ingest", "git_capture",
})
...
def build_golden(entries: list[dict], max_expected: int = 5,
    ...
    ⚠️ После введения _BOUNDARY_TOOLS окно по времени стало ИЗБЫТОЧНЫМ: у валидных пар
    максимальный разрыв 552 с, то есть отсекать уже нечего. Параметр оставлен как
    инструмент для будущих замеров, но включать его без нужды не следует — он режет
    по величине, а не по смыслу.
    """
    golden: list[dict] = []
    ...
    for e in entries:
        ...
        if not isinstance(args, dict):
            args = {}
        if tool in _BOUNDARY_TOOLS:
            # Работа сменилась — всё, что откроют дальше, к прошлому запросу не относится.
            flush()
            continue
...
def per_query_rr(golden: list[dict], retrieve, limit: int = 10) -> list[float]:
    """Обратный ранг по КАЖДОМУ запросу (0.0 при промахе), в порядке golden.

    Нужен для парного сравнения конфигураций: агрегат прячет то, из чего он сложен.
    """
    out = []
    for item in golden:
        ranked = retrieve(item["query"], item["project"], limit) or []
        rank = next((i for i, p in enumerate(ranked[:limit], 1) if p in item["expected"]), None)
        out.append(1.0 / rank if rank else 0.0)
    return out


def compare(golden: list[dict], retrieve_a, retrieve_b, limit: int = 10,
            resamples: int = 10000, seed: int = 42) -> dict:
    """Сравнить две конфигурации ЧЕСТНО: ΔMRR + доверительный интервал + счёт по запросам.

    Зачем это отдельная функция, а не «посмотреть, у кого MRR выше».

    1. Разница средних НЕ означает улучшения. Замер 2026-07-19: отключение
       семантического канала давало MRR 0.5493 против 0.5212 — выглядело победой.
       Но по запросам стало ХУЖЕ 27 и лучше 25: прирост держался на нескольких
       крупных улучшениях, перекрывавших чуть более многочисленные ухудшения.
       Такая конфигурация выигрывает на бумаге и проигрывает в работе, и увидеть
       это можно ТОЛЬКО в разбивке по запросам.

    2. Разницу нужно сопоставлять с её собственной неопределённостью. Тот же замер:
       ΔMRR +0.0281 с 95%-м интервалом [−0.0158, +0.0738] — ноль внутри, принимать
       нельзя. Бутстрэп по запросам даёт интервал, не требуя предположений о
       распределении (метрика ограничена сверху и снизу, нормальность тут неуместна).

    3. Бинаризация по recall@1 (McNemar) теряет мощность: переход с 5-го места на
       2-е — реальное улучшение, а в счёте «попал/не попал» его нет. Поэтому здесь
       бутстрэп по обратным рангам, а счёт лучше/хуже идёт рядом, а не вместо.

    Возвращает: n, mrr_a, mrr_b, delta, ci_low, ci_high, significant (ДИ не
    накрывает ноль), better/worse/unchanged (число запросов, где ранг изменился).

    seed фиксирован: два прогона на одних данных обязаны давать одинаковый вывод,
    иначе «значимость» сама станет источником шума.
    """
    rr_a = per_query_rr(golden, retrieve_a, limit)
    rr_b = per_query_rr(golden, retrieve_b, limit)
    n = len(golden)
    if not n:
        return {"n": 0, "mrr_a": 0.0, "mrr_b": 0.0, "delta": 0.0, "ci_low": 0.0,
                "ci_high": 0.0, "significant": False, "better": 0, "worse": 0,
                "unchanged": 0}

    diff = [b - a for a, b in zip(rr_a, rr_b)]
    delta = sum(diff) / n
    rng = random.Random(seed)
    boot = sorted(sum(diff[rng.randrange(n)] for _ in range(n)) / n
                  for _ in range(resamples))
    lo, hi = boot[int(resamples * 0.025)], boot[int(resamples * 0.975)]

    return {
        "n": n,
        "mrr_a": round(sum(rr_a) / n, 4),
        "mrr_b": round(sum(rr_b) / n, 4),
        "delta": round(delta, 4),
        "ci_low": round(lo, 4),
        "ci_high": round(hi, 4),
        "significant": lo > 0 or hi < 0,
        "better": sum(1 for a, b in zip(rr_a, rr_b) if b > a),
        "worse": sum(1 for a, b in zip(rr_a, rr_b) if b < a),
        "unchanged": sum(1 for a, b in zip(rr_a, rr_b) if b == a),
    }


def format_comparison(res: dict, label_a: str = "A", label_b: str = "B") -> str:
    """Однострочный вывод сравнения — чтобы отчёт нельзя было прочитать выборочно."""
    verdict = "ЗНАЧИМО" if res["significant"] else "не значимо (ДИ накрывает ноль)"
    warn = ""
    if res["delta"] > 0 and res["worse"] >= res["better"]:
        warn = "  ⚠️ средний прирост при БОЛЬШЕМ числе ухудшений — не принимать"
    return (f"{label_a} {res['mrr_a']} → {label_b} {res['mrr_b']}  "
            f"Δ={res['delta']:+.4f}  ДИ [{res['ci_low']:+.4f}, {res['ci_high']:+.4f}]  "
            f"лучше/хуже {res['better']}/{res['worse']}  {verdict}{warn}")
