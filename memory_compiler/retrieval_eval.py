...
# по релевантности ВООБЩЕ: он перебирает файлы, точно сверяет тег в строке `**Теги:**`
# и упорядочивает совпадения по дате статьи, до v1.91.0 — по обходу каталога
# (handlers_search.py). «Первого места» по релевантности там нет, поэтому выбор статьи
# из такого списка ничего не говорит о порядке выдачи.
#
