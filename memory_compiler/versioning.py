...
def is_date_like(v) -> bool:
    """X.Y.Z, похожее на календарную дату: ГГГГ.ММ.ДД (2024.06.25) или ДД.ММ.ГГГГ
    (15.09.2026); год 2000-2099, месяц 1-12, день 1-31. Такие строки — даты, не версии
    (иначе год >> мажор и дата «максимальна»).
    ДД.ММ.ГГГГ добавлен после живой порчи 15.09.2026: русская дата проходила как версия,
    и finish_task затёр трекеры 0.4.0 → 15.09.2026. Год — только четырьмя цифрами
    (15.09.26 неотличимо от версии). Версия вида 1.2.2024 тоже считается датой: промах
    авто-апдейта безвреднее порчи трекера.
    """
    parts = str(v).split(".")
    if len(parts) != 3:
        return False
    try:
        first, month, last = int(parts[0]), int(parts[1]), int(parts[2])
    except ValueError:
        return False
    if not 1 <= month <= 12:
        return False
    if 2000 <= first <= 2099 and 1 <= last <= 31:
        return True
    return len(parts[2]) == 4 and 2000 <= last <= 2099 and 1 <= first <= 31
