...
def test_search_by_tag_is_not_a_source():
    ...
    Он перебирает файлы, точно сверяет тег и упорядочивает совпадения по дате статьи,
    а не по релевантности — «первого места» там нет, поэтому клик из такого списка
    ничего не говорит о качестве ранжирования."""
    golden = build_golden([
