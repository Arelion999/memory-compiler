...
`supersedes` при сохранении ставит пометку в опровергнутую статью (сторона поправки
не хранится, а вычисляется по этой пометке), чтение опровергнутой статьи показывает
предупреждение, а выдача подтягивает поправку, даже если та не прошла по
релевантности.
"""
...
def test_superseded_article_links_to_correction(proj, tmp_path):
