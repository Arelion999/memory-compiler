...
- **Регламент релиза: деплой + живая верификация теперь ДО коммита/тега/пуша.** Раньше тег ставился, а живой деплой шёл после — «релиз» мог висеть на GitHub, не будучи проверенным в бою (особенно для средовых пересборок образа, что не ловятся тестами). Новый порядок в `внутренний регламент релиза` (шаг 6 «Деплой + ЖИВАЯ верификация» перед коммитом; философия «тег = проверено живым деплоем, не только тестами»). `scripts/release.ps1|sh`: подтверждение перед коммитом переформулировано — требует сначала задеплоить рабочее дерево (VERSION уже поднят) и проверить `/api/health` (+ /mcp handshake при смене транспорта), и только на `y` делать коммит/тег/push. Изменение чисто документарное/скриптовое — в контейнер не идёт, деплой N/A.
...
- **Регламент релиза — `внутренний регламент релиза`.** Единый процесс для любого изменения: код+тесты → документация → CHANGELOG → VERSION → проверка на утечки (`git audit-secrets` + `gitleaks`) → коммит по-русски → аннотированный тег → `git push --follow-tags`. Инвариант согласованности: `VERSION` == верхняя секция `CHANGELOG.md` == git-тег == версия в сообщении коммита. Один релиз = один коммит = один тег.
