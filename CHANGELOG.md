...
FP всплыл на реальной задаче mot_ut: версия расширения 1С `0.2.0.4 → 0.2.0.5` распозналась как IP и дала «противоречие» против `192.168.25.55`. Причина — 4-частная версия (октеты ≤255) совпадает с IP-маской, а IP-паттерн в `_extract_facts` идёт ПЕРЕД «версией» (v1.7.11 не помогал — значение попадало в метку IP, не «версия»). Полный разбор класса вскрыл ещё пробелы: версии с октетом >255 и query-разница в URL.
...
При каждом `save_lesson`/`finish_task` функция сравнивала embedding темы новой статьи со ВСЕМИ статьями базы и дописывала «См. также» в каждую с `0.55 < sim < 0.85` — без потолка и без скоупа по проекту. Деплой-раннбук memory-compiler оброс ~200 ссылками на чужие проекты (mira, antilopadv, mot_zup). Каждый последующий `edit_article` метил десятки «зависимых» статей (write amplification, git-churn в KB-репозитории).
