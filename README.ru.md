...
Результаты поиска рисуются интерактивной панелью прямо в чате, а не стеной текста. Это расширение `io.modelcontextprotocol/ui` (спека 2026-01-26): инструмент `search` несёт `_meta.ui.resourceUri`, хост забирает `ui://memory-compiler/search-results.html` и рендерит его в песочном iframe, общаясь с ним по JSON-RPC поверх `postMessage`.
