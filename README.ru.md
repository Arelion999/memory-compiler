...
⚠️ Кириллица дорогая по токенам. На дефолтном потолке держите `content` в пределах ~1500 символов, на поднятом спокойно проходит ~4000. Длинный разбор лучше сохранять отдельной статьёй и ссылаться на неё: заголовки весят в поиске ×5, а текст, закопанный в `content` чужой темы, по своей теме не находится.
